...
from reinforce.dmc_v1.dyg import DMCTrainer
from reinforce.env.factory import registration

def dmc_train(flags):
	# 创建环境
	env = registration(flags.env)
	# 初始化dmc训练模型
	trainer = DMCTrainer(
		env,
	...
	# 训练 monster dmc_v1 agents
	trainer.start()


if __name__ == '__main__':
	parser = argparse.ArgumentParser("DMC monster in RLCard")
	parser.add_argument(
		'--env',
		type=str,
		default='monster',
	...
	parser.add_argument(
		'--x_pid',
		default='monster',
		help='训练编号(default: monster)')

	parser.add_argument(
		'--save_dir',
		default='results/deep_mc_v1/monster_pick',
		help='保存训练数据的根目录')
