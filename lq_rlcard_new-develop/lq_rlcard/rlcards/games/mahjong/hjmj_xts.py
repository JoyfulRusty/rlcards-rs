...
class HJMoveGenerator:
            ...
            def optimal_split_cards(hand_cards):
                ...
                if not all_split_cards:
                    ...
                    for two in two_list:
                        if two == pair:
                            continue
                        two_val = HJMoveGenerator.get_value(two)
                        if two_val == 1:
                    ...
                    for comb in all_comb:
                        ...
                        if comb_list:
                            ...
                            for sc in res_split_cards:
                                comb_type = HJMoveGenerator.judge_cards_type(sc)
        ...
        for i, path in enumerate(optimal_path):
            ...
            for j, cards in enumerate(path[:yx_heap + 1]):
                ...
                if cards_len == 1:
                    card = cards[0]
                    yxp_cards.add(card)
                    card_val = HJMoveGenerator.get_value(card)
                    if xts != 0:
                        # 计算有效牌(先处理边界，再处理中间)
                        if card_val == 1:
                    ...
                    # 计算二连顺(只处理边界)
                        ...
                        card1_val = HJMoveGenerator.get_value(card1)
                        card2_val = HJMoveGenerator.get_value(card2)
                        if card1_val != 1:
    ...
    def get_best_card(self, record_chu_pai: dict, ping_hu=False):
        ...
        for card, yxp in record_chu_pai.items():
            # print(f"打{card}, 有效牌：{yxp}, {len(yxp)}种")
            count = 0
            for c in yxp:
    ...
    def get_repeat_cards(cards):
        ...
        """
        result = []
        card_to_count = HJMoveGenerator.cards_to_count(cards)
