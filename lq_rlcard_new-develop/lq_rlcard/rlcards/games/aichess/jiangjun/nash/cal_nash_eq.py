...
class NashEq:
	...
	"""
	def __init__(self):
		self.__name = 'cal_Nash_equilibrium'  # 计算纳什平衡
	...
	def cal_rate(battle_dir_path):
		...
		todo: battle_dir_path 单个nash battle对局
		--- model_1
	...
	def cal_nash_eq(rate_tables):
		...
		eth_a = 0.1
		epsilon_bar = 10 ** -1
		play_counts = tuple(game.stochastic_fictitious_play(iterations=iterations, etha=eth_a, epsilon_bar=epsilon_bar))
		prb_row = [
			row_play_counts / np.sum(row_play_counts) if np.sum(row_play_counts) != 0 else row_play_counts + 1 / len(row_play_counts) 
			for (row_play_counts, col_play_counts), _ in play_counts][-1]
		
		prb_col = [
			col_play_counts / np.sum(col_play_counts) if np.sum(col_play_counts) != 0 else col_play_counts + 1 / len(col_play_counts) 
			for (row_play_counts, col_play_counts), _ in play_counts][-1]
	...
	def rpp(
		...
		rate_tables = - rate_tables.T  # 行是新的移动，列是最后的移动
	...
	def build_rate_table(self, model_pool_list, latest_model_name, local_nash_battle_dir):
		"""
		todo: 建立并保存胜率表并返回纳什均衡
		model_pool_list: nash对战模型列表，按时间排序，最新的在最后latest_model_name
		...
		battle_models = copy.deepcopy(model_pool_list)
		battle_models.extend([latest_model_name])
		battle_models.sort(reverse=True)
		table = {}
		for model_index in range(len(battle_models) - 1):
			player1 = battle_models[model_index]
			player1_path = os.path.join(local_nash_battle_dir, player1)
			for player2 in battle_models[model_index + 1:]:
				battle_dir_path = os.path.join(player1_path, player2)
				if os.path.isdir(battle_dir_path):
					# 玩家 1 始终是新玩家
					player1_win_rate, player2_win_rate = self.cal_rate(battle_dir_path)
		...
		players = copy.deepcopy(battle_models)
		players.sort()
