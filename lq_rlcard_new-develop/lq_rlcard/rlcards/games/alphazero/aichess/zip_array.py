...
def array2num(array):
    ...
    转换array
    """
    return list(filter(lambda string: all(num2array[string] == array), num2array))[0]
...
#
def zip_state_mct_prob(tuple):
    """
    (state, mcts_prob, winner) ((9,10,9),2086,1) => ((9,90),(2,1043),1)
    """
    state, mct_prob, winner = tuple
    state = state.reshape((9,-1))
    mct_prob = mct_prob.reshape((2,-1))
    state = zip_array(state)
    mct_prob = zip_array(mct_prob)

    return state, mct_prob, winner

def recovery_state_mct_prob(tuple):
    state, mct_prob, winner = tuple
    state = recovery_array(state)
    mct_prob = recovery_array(mct_prob)
    state = state.reshape((9,10,9))
    mct_prob = mct_prob.reshape(2086)
...
def recovery_array(array, data=0.):
    ...
    for i in range(array[0][0]):
        recovery_res.append([data for _ in range(array[0][1])])
