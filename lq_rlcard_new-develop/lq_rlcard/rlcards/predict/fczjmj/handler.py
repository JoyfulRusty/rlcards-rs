...
from rlcards.games.mahjong.fczj_xts import FCZJMoveGenerator
from rlcards.games.mahjong.yxp import calc_best_cards_by_hfc, calc_best_cards_by_xxc

class FCZJMahjongHandler(BaseServer):
    """
    麻将处理函数
    """
    ...
    async def cal_action(self, data: dict):
        ...
        # 计算发财捉鸡出牌
        action = self.calc_fc_actions(data)
    ...
    async def cal_pong(self, data: dict):
        """
        计算是否碰
        """
        await self.info_log(data)  # 日志记录
    ...
    async def cal_gang(self, data: dict):
        """
        计算是否杠
        """
        await self.info_log(data)  # 日志记录
    ...
    async def cal_yxp(self, data: dict):
        """
        计算机器人摸好牌
        """
        await self.info_log(data)  # 日志记录
    ...
    def share_server(server_name):
        ...
        发送服务
        """
        return FCZJMahjongHandler(server_name)

    @staticmethod
    def calc_remain_cards(curr_hand_cards: list, remain_cards: dict):
        """
        统计剩余卡牌
        """
        # 出牌、碰牌、杠牌已经减去，此处不再计算
    ...
    def calc_xxc_actions(self, data: dict):
        """
        计算休闲场出牌
        """
        print("============计算休闲场出牌============")
        ...
        mg.update_attr(
            ...
            data.get("left_count"),
            data.get("others_hand_cards"),
            data.get("magic_card"),  # 判断玩家当前手牌是否存在万能牌
            remain_cards
        )
    ...
    def calc_hfc_actions(self, data: dict):
        """
        计算话费场出牌
        """
        print("============计算话费场出牌============")
    ...
    def calc_fc_actions(self, data: dict):
        """
        计算发财捉鸡出牌
        """
        print("============计算发财捉鸡出牌============")
