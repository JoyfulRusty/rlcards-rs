...
class FCZJMoveGenerator:
    ...
    def update_attr(
        ...
        self.magic_card = magic_card or None  # 万能牌
        self.__hand_cards_len = len(self.__hand_cards)  # 手牌数量
    ...
    def build_qys_types(self, ting_cards=None, no_dian_pao=False):
        ...
        ting_cards = ting_cards or []
        print("当前手牌和数量: ", self.__hand_cards, self.__hand_cards_len)
        args = self.calc_cards_list_by_count()  # 此接口不包含万能牌的计算和统计
        all_cards = []
        all_xts_yxp = []
    ...
    def select_hu_type(self, all_xts_yxp, ph_best_cards):
        ...
        if self.left_count < (self.modify_flag + 1) // 2:
            ...
            return self.control_best_cards(min_xts[0][2], self.calc_all_xts_best_cards(all_xts_yxp))
    ...
    def calc_xts_by_best_hu_type(self):
        ...
        if len(all_xts_yxp) == 1:
            ...
            if all_xts_yxp[0][1] < 0:
                return random.choice(best_cards1)
            return self.control_best_cards(best_cards1, self.calc_all_xts_best_cards(all_xts_yxp))
        # 存在多种牌型
        # 1.计算向听数相差2范围内牌型打出
    ...
    def cal_xts_ping_hu(self, *args, need_mz=None):
            ...
            def optimal_split_cards(hand_cards):
                ...
                # 当前卡牌已被切割重组
                    ...
                    for two in two_list:
                        if two == pair:
                            continue
                        two_val = FCZJMoveGenerator.get_value(two)
                        if two_val == 1:
                    ...
                    for comb in all_comb:
                        ...
                        if comb_list:
                            ...
                            for sc in res_split_cards:
                                comb_type = FCZJMoveGenerator.judge_cards_type(sc)
                            ...
                            # todo: 发财捉鸡不再计算向听数相等时的最优切割，只计算最小向听数的情况
                            # 判断当前向听数是否减少，并添加切割卡牌至最优组合中
        ...
        # todo: 计算发财捉鸡当前最优出牌(添加万能牌后结果)
        # res_xts, best_cards = self.cal_yxp_by_ping_hu_fc_zj(optimal_path, need_heap, record_lowest_xts)
        res_xts, best_cards = self.calc_add_magic_card_by_result(record_lowest_xts, optimal_path, need_heap)

        return res_xts, best_cards

    @staticmethod
    def check_path_5_by_xts(path: list, need_heap: int):
        """
        检查已经划分好的前5个组合是否全部组合完成
        [[11], [111], [123], [111], [123]]
        将牌 + 刻子 + 顺子
        返回1，说明当前万能牌可用，可减少向听数
        """
        # 判断当前对子是否还能使用万能牌
        for sub_path in path:
            # 判断顺子或刻子是否还能使用万能牌
            # 二连顺和间隔顺都减去一个向听数
            for s_path in sub_path[1: need_heap + 1]:
                if len(s_path) < 3:
                    return 1
            # 组成对子也减去一个向听数
            if len(sub_path[0]) == 1:
    ...
    def calc_add_magic_card_by_result(self, xts: int, path: list, need_heap: int):
        """
        计算添加万能牌(发财)之后组合结果，并计算出最小向听数
        """
        # 组合将牌[1]、顺子或刻子[2]、无组合[0]
        path_5_xts = self.check_path_5_by_xts(path, need_heap)

        # 更新使用万能牌后的向听数
        xts = xts - path_5_xts

        # todo: 1.计算加入万能牌之后是否形成胡牌(向听数小于等于0时)
        if xts < 0:
            print("当前卡牌形成胡牌或听牌，重新计算最优出牌~")
            return xts, self.calc_hu_xts_by_best_card(path)

        # todo: 2.计算使用万能牌之后不能形成胡牌，则计算对应的最优出牌
        # 当前向听数无组合时，说明前5个组合已全部组合完必
        # 则计算剩余的卡牌组合，并从剩余卡牌中选择最优的当前出牌
        elif xts != 0:
            return self.cal_yxp_by_ping_hu_fc_zj(path, need_heap, xts)

        # todo: 3.当前向听数为0时，则选择最优出牌即可
        else:
            for sub_path in path:
                # 计算剩余什么都不能组合的卡牌
                # 将其与万能牌进行对应的组合
                remain_path = sum(sub_path[need_heap + 1:], [])
                if not remain_path:
                    return xts, sum(sub_path[need_heap - 1:])
                return xts, remain_path
        ...
        for i, path in enumerate(optimal_path):
            ...
            for j, cards in enumerate(path[:yx_heap + 1]):
                ...
                if cards_len == 1:
                    card = cards[0]
                    yxp_cards.add(card)
                    card_val = FCZJMoveGenerator.get_value(card)
                    if xts != 0:
                        # 计算有效牌(先处理边界，再处理中间)
                        if card_val == 1:
                    ...
                    # 计算二连顺(只处理边界)
                        ...
                        card1_val = FCZJMoveGenerator.get_value(card1)
                        card2_val = FCZJMoveGenerator.get_value(card2)
                        if card1_val != 1:
    ...
    def calc_hu_xts_by_best_card(self, path: list):
        """
        发财反鸡向听数小于0时，计算形成胡牌后最佳出牌
        """
        # todo: 优先选择万能牌作为将牌
        # 当万能牌与单张作为将牌时
        # 则优先选择将单张将牌打出
        ...
        for sub_path in path:
            if len(sub_path[0]) == 1:
                return sub_path[0]
        # 当前万能牌不作为将牌时，只能是顺子或刻子组合
        # 则根据当前手牌选择用于计算的最终所选出牌结果
        best_cards = self.choice_best_cards(path)
        # 从计算好出牌牌组中，选择计算后的最佳出牌卡牌
        return self.calc_hu_by_best_cards(best_cards)

    def choice_best_cards(self, path: list, lai_zi=51):
        """
        选择用于计算最终出牌的牌组
        """
        # 万能牌充当顺子或刻子时
        # 选择移除万能牌后的牌组
        if lai_zi in self.__hand_cards:
            for sub_path in path:
                return sub_path[-1]
        # 万能牌未充当牌组时，则从手牌中进行选择
        return self.__hand_cards

    def calc_hu_by_best_cards(self, best_cards):
        """
        计算形成胡牌后的最终出牌
        """
        # todo: 万能牌不作可选最优出牌
        ...
        for card in best_cards:
            ...
            if tmp_count > count:
                res_card.clear()
                count = tmp_count
                res_card.append(card)

        # 无最优出牌时，则返回当前卡牌
        if not res_card:
            return self.__hand_cards

        return list(set(res_card))

    def cal_yxp_by_ping_hu_fc_zj(self, optimal_path: list, yx_heap: int, xts: int):
        """
        计算发财捉鸡有效牌
        """
        # 判断向听数是否小于0
        if xts < 0:
            return xts, self.calc_hu_xts_by_best_card(optimal_path)
        # 胡牌类型组的有效卡牌
        record_chu_pai = {}
        for i, path in enumerate(optimal_path):
            yxp_cards = set()
        ...
        return xts, self.get_best_card(record_chu_pai, ping_hu=True)

    def get_best_card(self, record_chu_pai: dict, ping_hu=False):
        """
        此接口计算不包含万能卡牌，因出牌可选中不包含万能牌
        """
        yxp_count = 0
        best_cards = []
    ...
    def get_repeat_cards(cards):
        ...
        """
        result = []
        card_to_count = FCZJMoveGenerator.cards_to_count(cards)
