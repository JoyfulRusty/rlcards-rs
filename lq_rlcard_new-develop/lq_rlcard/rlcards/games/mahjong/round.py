...
from rlcards.games.mahjong.policy import calc_play_cards
...
class MahjongRound:
	...
	def get_state(self, players, player_id, other_hand_cards, hand_card_nums):
		...
		# 出牌(11, 12...)
			...
			# TODO: 天听或锁牌状态需要重新计算出牌，已经锁住的牌，不能打出
			if curr_player.lock_cards:
				state['actions'] = curr_player.out_of_lock_cards()
			else:
				state['actions'] = calc_play_cards(curr_player.curr_hand_cards, const.LAI_ZI)
				if not state['actions']:
					state['actions'] = curr_player.curr_hand_cards

		return state

	def get_state_bak(self, players, player_id, other_hand_cards, hand_card_nums):
		"""
		获取玩家状态信息
		"""
		state = {
