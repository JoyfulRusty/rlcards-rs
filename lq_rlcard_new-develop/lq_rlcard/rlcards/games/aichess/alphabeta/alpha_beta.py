...
from rlcards.games.aichess.alphabeta.step import Step
from rlcards.games.aichess.alphabeta.chess import Chess
from rlcards.games.aichess.alphabeta.board import ChessBoard
from rlcards.games.aichess.alphabeta.relation import Relation
from rlcards.games.aichess.alphabeta.history import HistoryCache
...
class AlphaBeta:
	...
	def __init__(self):
		...
		初始化α-β剪枝属性参数
		"""
		self.board = ChessBoard()  # 游戏对局棋盘
		self.max_depth = const.max_depth  # 最大搜索深度
		self.history = HistoryCache()  # 启发式缓存表
		self.best_move = Step()  # 更新移动move
		self.cnt = 0

	def do_alpha_beta(self, depth: int, alpha: int, beta: int):
		"""
		todo: alpha-beta剪枝
			1.alpha是最大可能下界
			2.beta是最小可能上界
		"""
		who = (self.max_depth - depth) % 2  # 玩家ID
		# todo: 搜索到指定深度则不再进行搜索
		'''
		...
		for i in range(len(move_list)):
			move_list[i].score = self.history.get_history_score(who, move_list[i])
		# 为了更容易剪枝利用历史表得分进行排序
		move_list.sort()
		...
		for step in move_list:
			...
			score = -self.do_alpha_beta(depth - 1, -beta, -alpha)
			# 添加分数
			...
			'''
			Min分支返回给Max分支的最小分值其实就是当前Max分支的分值，所以需要更新的是α值，同样的，Max分支会选择最大的一个α值返回给Min分支，
			这个α值也就是当前Min分支的分值，所以需要更新的是β值，只需要记住一点：α值的来源是β，β值的来源是α(除了叶子结点，因为叶子结点的值来源于评估函数)
			'''
			# 当极大值 > 极小值时，则进行剪枝
			'''
		...
		if best_step.from_x != -1:
			self.history.add_history_score(who, best_step, depth)
		return alpha
			...
			for y in range(10):
				...
				if num_attacked == 0:
					...
					if num_guarded == 0:  # 如果没有保护
						...
						elif num_guarded == 1 and num_attacked > 1 and min_attack < const.base_val[
							now_chess_type] + sum_guard:
							tmp_relation_val[now_belong] -= mut_val * unit_val
...
if __name__ == "__main__":
	game = AlphaBeta()
	while True:
		from_x = int(input("输入原坐标x: "))
		from_y = int(input("输入原坐标y: "))
		to_x = int(input("输入移动坐标x: "))
		to_y = int(input("输入移动坐标y: "))
		s = Step(from_x, from_y, to_x, to_y)
		game.do_alpha_beta(game.max_depth, const.min_val, const.max_val)
		print("输出最佳移动: ", game.best_move)
