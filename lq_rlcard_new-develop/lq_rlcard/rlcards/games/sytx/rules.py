...
from rlcards.const.sytx.const import *
...
class Rules(metaclass=Singleton):
	...
	def abstract_values(cards: list):
		...
		提取卡牌值
		"""
		result = list(map(Rules.value, cards))
		return result
	...
	@staticmethod
	def abstract_value_to_dan_zhang_by_sums(cards):
		pass

	@staticmethod
	def calc_big_small_by_values(cards):
		"""
		计算单张卡牌值
		"""
		big_cards = []
	...
	def is_bigger_old(player_card, deal_card, compare_num=-1):
		...
		if compare_num == COMPARE_Z_ZOU_X_XIN_NUM:  # 庄走闲信
			...
			return Rules.is_bigger_by_lz(x_cards, z_cards, compare_num)
		elif compare_num == COMPARE_Z_AND_X_SY:  # 庄闲水鱼
			result = Rules.compare_sy(x_cards, z_cards)
			if result == IS_DRAW:
				return DRAW
	...
	@staticmethod
	def is_bigger(player_card, deal_card, compare_num=-1, is_gx=True):
		"""
		癞子版比较只需传with_lai_zi参数即可，经典版本无需传
		with_lai_zi: 癞子牌 <-> [515, 516], poker类中有定义
		is_gx: 是否是广西玩法
	...
	@staticmethod
	def is_bigger_by_lz(player_card, deal_card, compare_num=-1):
		"""
		有癞子牌存在比大小
