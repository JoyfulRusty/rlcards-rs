...
class ChessBoard:
	...
	def __init__(self):
		...
		for i in range(9):
			...
			for j in range(10):
				# 为0则为空
				if const.init_board[i][j] == 0:
					self.board[i].append(Chess(-1, const.init_board[i][j]))
				else:
					# [0/1]玩家
					self.board[i].append(Chess(0 if j < 5 else 1, const.init_board[i][j]))
	...
	def IsKingFaceToFace(self, x, y, who):
		...
		if who == 1:
			for i in range(9, 6, -1):
				if self.board[x][i].chess_type == const.jiang:
					a = True
					b = i
			if not a:
				return a
			for j in range(y + 1, b):
				if self.board[x][j].chess_type != const.kong:
					return False
			return b
		# 否则为机器人
		else:
