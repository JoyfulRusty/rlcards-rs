...
def get_better_combine_by_3(cards: list, other_cards: list = None):
	""" 根据3张牌扯牌（机器人使用） """
	if not cards or len(cards) != 3:  # [1,2,3] -> [123] [132] [231]
		return cards
	if not other_cards:
		other_cards = []
	cards_copy = cards[:]
	cards_copy.sort(key=Rules.value)
	v_count_dict, card_v_list = Rules.get_count_list_by_value(cards_copy)
	# 3张都一样: 则不用扯
	v_count_dict_len = len(v_count_dict)
	if v_count_dict_len == 1:
		return cards

	all_combine = list(itertools.combinations(cards_copy, 2))
	cards_v = Rules.abstract_value_to_dan_zhang(card_v_list)
	all_combine_val = list(itertools.combinations(cards_v, 2))
	cards_v_arr = np.array(all_combine_val)
	# 1个对子 + 1张
	if v_count_dict_len == 2:
		comb_index = np.argwhere(cards_v_arr[:, 0] == cards_v_arr[:, 1])[0, 0]
	else:
		# 都是散牌
		per_pu_sum = np.round(cards_v_arr.sum(1) % 10, decimals=1)
		max1_index = np.where(per_pu_sum == max(per_pu_sum))[0][0]
		if max1_index == 0:
			if per_pu_sum[1] > per_pu_sum[2]:
				max2_index = 1
			else:
				max2_index = 2
		elif max1_index == 1:
			if per_pu_sum[0] > per_pu_sum[2]:
				max2_index = 0
			else:
				max2_index = 2
		else:
			if per_pu_sum[0] > per_pu_sum[1]:
				max2_index = 0
			else:
				max2_index = 1

		comb_index = random_choice_card([max1_index, max2_index], [0.9, 0.1])

	ensure_comb = list(all_combine[comb_index])
	for c in ensure_comb:
		cards_copy.remove(c)
	ensure_comb.extend(cards_copy)
	return ensure_comb