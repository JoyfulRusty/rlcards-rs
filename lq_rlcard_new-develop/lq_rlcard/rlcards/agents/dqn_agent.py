...
class DQNAgent(object):
	...
	def train(self):
		...
		q_value_next = self.q_estimator.predict_no_grad(next_state_batch)
		legal_actions = []
		for b in range(self.batch_size):
			legal_actions.extend([i + b * self.num_actions for i in legal_action_batch[b]])
		masked_q_values = -np.inf * np.ones(self.num_actions * self.batch_size, dtype=float)
		masked_q_values[legal_actions] = q_value_next.flatten()[legal_actions]
...
class Estimator(object):
	...
	Q值估计器，该网络用于Q网络和目标网络
	"""
	def __init__(self, num_actions=2, learning_rate=0.005, state_shape=None, mlp_layers=None, device=None):
		"""
		初始化Estimator属性参数
		"""
		self.device = device
